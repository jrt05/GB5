...
void GFXs::create() {
    ...
    setGraphicsMode(currentGraphicsMode);
...
void GFXs::setGraphicsMode(int mode) {
    if (mode == NEAREST || mode == BILINEAR) {
        if (mode == NEAREST) {
            ...
            SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "1");        // turn on nearest pixel sampling
...
bool GFXs::isGraphicsModeSet(int mode) {
    bool ret = false;
    if (mode == NEAREST || mode == BILINEAR) {
        ret = currentGraphicsMode == mode;
    }

    return ret;
}

void GFXs::setScale(int s) {
